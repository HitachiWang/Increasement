...
for s=1:4:k-3
    ...
    %读取ECWMF数据并平均
    TSdata_E=ncread(filename,'ts',[1 1 1 5],[144 73 9 1]); 
...
for s=2:4:k-2
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    TSdata_E=ncread(filename,'ts',[1 1 1 2 ],[144 73 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    TS_E=reshape(TSdata_E,[144 73 9]);
    TS_E_1=flipud(rot90(mean(TS_E,3)));%画图格式[73 144 time]
    TS_05_E(:,:,(s+2)/4)=TS_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    TSdata_I=ncread(filename,'ts',[1 1 10 2],[144 73 9 1]);
    TS_I=reshape(TSdata_I,[144 73 9]);
    TS_I_1=flipud(rot90(mean(TS_I,3)));
    TS_05_I(:,:,(s+2)/4)=TS_I_1(:,:);
    %读取Meteo-France数据并平均
    TSdata_M=ncread(filename,'ts',[1 1 19 2],[144 73 9 1]);
    TS_M=reshape(TSdata_M,[144 73 9]);
    TS_M_1=flipud(rot90(mean(TS_M,3)));
    TS_05_M(:,:,(s+2)/4)=TS_M_1(:,:);
    %读取UK_Met_Office数据并平均
    TSdata_U=ncread(filename,'ts',[1 1 28 2],[144 73 9 1]);
    TS_U=reshape(TSdata_U,[144 73 9]);
    TS_U_1=flipud(rot90(mean(TS_U,3)));
    TS_05_U(:,:,(s+2)/4)=TS_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    TSdata_C=ncread(filename,'ts',[1 1 37 2],[144 73 9 1]);
    TS_C=reshape(TSdata_C,[144 73 9]);
    TS_C_1=flipud(rot90(mean(TS_C,3)));
    TS_05_C(:,:,(s+2)/4)=TS_C_1(:,:);
    %读取所有Ensemble数据
    TSdata=ncread(filename,'ts',[1 1 1 2],[144 73 45 1]);
    TS=reshape(TSdata,[144 73 45]);
    TS_1=flipud(rot90(mean(TS,3)));
    TS_05(:,:,(s+2)/4)=TS_1(:,:);
end
...
for s=4:4:k
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    TSdata_E=ncread(filename,'ts',[1 1 1 8],[144 73 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    TS_E=reshape(TSdata_E,[144 73 9]);
    TS_E_1=flipud(rot90(mean(TS_E,3)));%画图格式[73 144 time]
    TS_11_E(:,:,(s+0)/4)=TS_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    TSdata_I=ncread(filename,'ts',[1 1 10 8],[144 73 9 1]);
    TS_I=reshape(TSdata_I,[144 73 9]);
    TS_I_1=flipud(rot90(mean(TS_I,3)));
    TS_11_I(:,:,(s+0)/4)=TS_I_1(:,:);
    %读取Meteo-France数据并平均
    TSdata_M=ncread(filename,'ts',[1 1  19 8],[144 73 9 1]);
    TS_M=reshape(TSdata_M,[144 73 9]);
    TS_M_1=flipud(rot90(mean(TS_M,3)));
    TS_11_M(:,:,(s+0)/4)=TS_M_1(:,:);
    %读取UK_Met_Office数据并平均
    TSdata_U=ncread(filename,'ts',[1 1 28 8],[144 73 9 1]);
    TS_U=reshape(TSdata_U,[144 73 9]);
    TS_U_1=flipud(rot90(mean(TS_U,3)));
    TS_11_U(:,:,(s+0)/4)=TS_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    TSdata_C=ncread(filename,'ts',[1 1 37 8],[144 73 9 1]);
    TS_C=reshape(TSdata_C,[144 73 9]);
    TS_C_1=flipud(rot90(mean(TS_C,3)));
    TS_11_C(:,:,(s+0)/4)=TS_C_1(:,:);
    %读取所有Ensemble数据
    TSdata=ncread(filename,'ts',[1 1 2 8],[144 73 45 1]);
    TS=reshape(TSdata,[144 73 45]);
    TS_1=flipud(rot90(mean(TS,3)));
    TS_11(:,:,(s+0)/4)=TS_1(:,:);
end
TS=rand(73,144,46,6,3);
%创建6个成员的集合 Var_TS
%填充20数据
TS(:,:,:,1,1)=TS_02_E;
TS(:,:,:,2,1)=TS_02_I;
...
ncid2=netcdf.create('./Ensemble/139EnsembleJunc.nc','clobber');
