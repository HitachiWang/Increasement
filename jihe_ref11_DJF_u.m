%this program is for jiheuybao U DJF ref=11
...
for s=4:4:k
    ...
    %读取ECWMF数据并平均
    Gdata=ncread(filename,'ua',[1 1 1 1 2],[144 73 5 45 3]); 
    %取相应模态的值 其中
    G_11=squeeze(mean(Gdata,5));  %mean DJF
    G_11_Ens(:,:,:,:,(s+0)/4)=G_11;  %按照时间填入（应该是+3/4）
end
for i=1:5
    k=1;
...
ncid2=netcdf.create('./Ensemble/131.ref11.DJF.nc','clobber');
