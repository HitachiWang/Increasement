...
for s=1:4:k-3
    ...
    %读取ECWMF数据并平均
    Udata_E=ncread(filename,'ua',[1 1 1 1 7],[144 73 1 9 1]); 
    ...
    %读取IFM-GEOM数据并平均
    Udata_I=ncread(filename,'ua',[1 1 1 10 7],[144 73 1 9 1]);
    U_I=reshape(Udata_I,[144 73 9]);
    U_I_1=flipud(rot90(mean(U_I,3)));
    U_02_I(:,:,(s+3)/4)=U_I_1(:,:);
    %读取Meteo-France数据并平均
    Udata_M=ncread(filename,'ua',[1 1 1 19 7],[144 73 1 9 1]);
    U_M=reshape(Udata_M,[144 73 9]);
    U_M_1=flipud(rot90(mean(U_M,3)));
    U_02_M(:,:,(s+3)/4)=U_M_1(:,:);
    %读取UK_Met_Office数据并平均
    Udata_U=ncread(filename,'ua',[1 1 1 28 7],[144 73 1 9 1]);
    U_U=reshape(Udata_U,[144 73 9]);
    U_U_1=flipud(rot90(mean(U_U,3)));
    U_02_U(:,:,(s+3)/4)=U_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    Udata_C=ncread(filename,'ua',[1 1 1 37 7],[144 73 1 9 1]);
    U_C=reshape(Udata_C,[144 73 9]);
    U_C_1=flipud(rot90(mean(U_C,3)));
    U_02_C(:,:,(s+3)/4)=U_C_1(:,:);
    %读取所有Ensemble数据
    Udata=ncread(filename,'ua',[1 1 1 1 7],[144 73 1 45 1]);
...
for s=3:4:k-1
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    Udata_E=ncread(filename,'ua',[1 1 1 1 1],[144 73 1 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    U_E=reshape(Udata_E,[144 73 9]);
    U_E_1=flipud(rot90(mean(U_E,3)));%画图格式[73 144 time]
    U_08_E(:,:,(s+1)/4)=U_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    Udata_I=ncread(filename,'ua',[1 1 1 10 1],[144 73 1 9 1]);
    U_I=reshape(Udata_I,[144 73 9]);
    U_I_1=flipud(rot90(mean(U_I,3)));
    U_08_I(:,:,(s+1)/4)=U_I_1(:,:);
    %读取Meteo-France数据并平均
    Udata_M=ncread(filename,'ua',[1 1 1 19 1],[144 73 1 9 1]);
    U_M=reshape(Udata_M,[144 73 9]);
    U_M_1=flipud(rot90(mean(U_M,3)));
    U_08_M(:,:,(s+1)/4)=U_M_1(:,:);
    %读取UK_Met_Office数据并平均
    Udata_U=ncread(filename,'ua',[1 1 1 28 1],[144 73 1 9 1]);
    U_U=reshape(Udata_U,[144 73 9]);
    U_U_1=flipud(rot90(mean(U_U,3)));
    U_08_U(:,:,(s+1)/4)=U_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    Udata_C=ncread(filename,'ua',[1 1 1 37 1],[144 73 1 9 1]);
    U_C=reshape(Udata_C,[144 73 9]);
    U_C_1=flipud(rot90(mean(U_C,3)));
    U_08_C(:,:,(s+1)/4)=U_C_1(:,:);
    %读取所有Ensemble数据
    Udata=ncread(filename,'ua',[1 1 1 1 1],[144 73 1 45 1]);
    U=reshape(Udata,[144 73 45]);
    U_1=flipud(rot90(mean(U,3)));
    U_08(:,:,(s+1)/4)=U_1(:,:);
end
...
for s=4:4:k
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    Udata_E=ncread(filename,'ua',[1 1 1 1 10],[144 73 1 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    U_E=reshape(Udata_E,[144 73 9]);
    U_E_1=flipud(rot90(mean(U_E,3)));%画图格式[73 144 time]
    U_11_E(:,:,(s+0)/4)=U_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    Udata_I=ncread(filename,'ua',[1 1 1 10 10],[144 73 1 9 1]);
    U_I=reshape(Udata_I,[144 73 9]);
    U_I_1=flipud(rot90(mean(U_I,3)));
    U_11_I(:,:,(s+0)/4)=U_I_1(:,:);
    %读取Meteo-France数据并平均
    Udata_M=ncread(filename,'ua',[1 1 1 19 10],[144 73 1 9 1]);
    U_M=reshape(Udata_M,[144 73 9]);
    U_M_1=flipud(rot90(mean(U_M,3)));
    U_11_M(:,:,(s+0)/4)=U_M_1(:,:);
    %读取UK_Met_Office数据并平均
    Udata_U=ncread(filename,'ua',[1 1 1 28 10],[144 73 1 9 1]);
    U_U=reshape(Udata_U,[144 73 9]);
    U_U_1=flipud(rot90(mean(U_U,3)));
    U_11_U(:,:,(s+0)/4)=U_U_1(:,:);
    %读取CMCC-BoloUna数据并平均
    Udata_C=ncread(filename,'ua',[1 1 1 37 10],[144 73 1 9 1]);
    U_C=reshape(Udata_C,[144 73 9]);
    U_C_1=flipud(rot90(mean(U_C,3)));
    U_11_C(:,:,(s+0)/4)=U_C_1(:,:);
    %读取所有Ensemble数据
    Udata=ncread(filename,'ua',[1 1 1 1 10],[144 73 1 45 1]);
    U=reshape(Udata,[144 73 45]);
    U_1=flipud(rot90(mean(U,3)));
    U_11(:,:,(s+0)/4)=U_1(:,:);
end
...
ncid2=netcdf.create('./Ensemble/131EnsembleApril.nc','clobber');
