...
for s=1:4:k-3
    ...
    %读取ECWMF数据并平均
    Gdata_E=ncread(filename,'g',[1 1 2 1 6],[144 73 1 9 1]); 
...
for s=2:4:k-2
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    Gdata_E=ncread(filename,'g',[1 1 2 1 3 ],[144 73 1 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    G_E=reshape(Gdata_E,[144 73 9]);
    G_E_1=flipud(rot90(mean(G_E,3)));%画图格式[73 144 time]
    G_05_E(:,:,(s+2)/4)=G_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    Gdata_I=ncread(filename,'g',[1 1 2 10 3],[144 73 1 9 1]);
    G_I=reshape(Gdata_I,[144 73 9]);
    G_I_1=flipud(rot90(mean(G_I,3)));
    G_05_I(:,:,(s+2)/4)=G_I_1(:,:);
    %读取Meteo-France数据并平均
    Gdata_M=ncread(filename,'g',[1 1 2 19 3],[144 73 1 9 1]);
    G_M=reshape(Gdata_M,[144 73 9]);
    G_M_1=flipud(rot90(mean(G_M,3)));
    G_05_M(:,:,(s+2)/4)=G_M_1(:,:);
    %读取UK_Met_Office数据并平均
    Gdata_U=ncread(filename,'g',[1 1 2 28 3],[144 73 1 9 1]);
    G_U=reshape(Gdata_U,[144 73 9]);
    G_U_1=flipud(rot90(mean(G_U,3)));
    G_05_U(:,:,(s+2)/4)=G_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    Gdata_C=ncread(filename,'g',[1 1 2 37 3],[144 73 1 9 1]);
    G_C=reshape(Gdata_C,[144 73 9]);
    G_C_1=flipud(rot90(mean(G_C,3)));
    G_05_C(:,:,(s+2)/4)=G_C_1(:,:);
    %读取所有Ensemble数据
    Gdata=ncread(filename,'g',[1 1 2 1 3],[144 73 1 45 1]);
    G=reshape(Gdata,[144 73 45]);
    G_1=flipud(rot90(mean(G,3)));
    G_05(:,:,(s+2)/4)=G_1(:,:);
end
...
for s=4:4:k
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    Gdata_E=ncread(filename,'g',[1 1 2 1 9],[144 73 1 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    G_E=reshape(Gdata_E,[144 73 9]);
    G_E_1=flipud(rot90(mean(G_E,3)));%画图格式[73 144 time]
    G_11_E(:,:,(s+0)/4)=G_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    Gdata_I=ncread(filename,'g',[1 1 2 10 9],[144 73 1 9 1]);
    G_I=reshape(Gdata_I,[144 73 9]);
    G_I_1=flipud(rot90(mean(G_I,3)));
    G_11_I(:,:,(s+0)/4)=G_I_1(:,:);
    %读取Meteo-France数据并平均
    Gdata_M=ncread(filename,'g',[1 1 2 19 9],[144 73 1 9 1]);
    G_M=reshape(Gdata_M,[144 73 9]);
    G_M_1=flipud(rot90(mean(G_M,3)));
    G_11_M(:,:,(s+0)/4)=G_M_1(:,:);
    %读取UK_Met_Office数据并平均
    Gdata_U=ncread(filename,'g',[1 1 2 28 9],[144 73 1 9 1]);
    G_U=reshape(Gdata_U,[144 73 9]);
    G_U_1=flipud(rot90(mean(G_U,3)));
    G_11_U(:,:,(s+0)/4)=G_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    Gdata_C=ncread(filename,'g',[1 1 2 37 9],[144 73 1 9 1]);
    G_C=reshape(Gdata_C,[144 73 9]);
    G_C_1=flipud(rot90(mean(G_C,3)));
    G_11_C(:,:,(s+0)/4)=G_C_1(:,:);
    %读取所有Ensemble数据
    Gdata=ncread(filename,'g',[1 1 2 1 9],[144 73 1 45 1]);
    G=reshape(Gdata,[144 73 45]);
    G_1=flipud(rot90(mean(G,3)));
    G_11(:,:,(s+0)/4)=G_1(:,:);
end
...
ncid2=netcdf.create('./Ensemble/129EnsembleJuly.nc','clobber');
