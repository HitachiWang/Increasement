...
for s=1:4:k-3
    ...
    %读取ECWMF数据并平均
    PRLRdata_E=ncread(filename,'prlr',[1 1 1 6],[144 73 9 1]); 
...
for s=2:4:k-2
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    PRLRdata_E=ncread(filename,'prlr',[1 1 1 3 ],[144 73 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    PRLR_E=reshape(PRLRdata_E,[144 73 9]);
    PRLR_E_1=flipud(rot90(mean(PRLR_E,3)));%画图格式[73 144 time]
    PRLR_05_E(:,:,(s+2)/4)=PRLR_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    PRLRdata_I=ncread(filename,'prlr',[1 1 10 3],[144 73 9 1]);
    PRLR_I=reshape(PRLRdata_I,[144 73 9]);
    PRLR_I_1=flipud(rot90(mean(PRLR_I,3)));
    PRLR_05_I(:,:,(s+2)/4)=PRLR_I_1(:,:);
    %读取Meteo-France数据并平均
    PRLRdata_M=ncread(filename,'prlr',[1 1 19 3],[144 73 9 1]);
    PRLR_M=reshape(PRLRdata_M,[144 73 9]);
    PRLR_M_1=flipud(rot90(mean(PRLR_M,3)));
    PRLR_05_M(:,:,(s+2)/4)=PRLR_M_1(:,:);
    %读取UK_Met_Office数据并平均
    PRLRdata_U=ncread(filename,'prlr',[1 1 28 3],[144 73 9 1]);
    PRLR_U=reshape(PRLRdata_U,[144 73 9]);
    PRLR_U_1=flipud(rot90(mean(PRLR_U,3)));
    PRLR_05_U(:,:,(s+2)/4)=PRLR_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    PRLRdata_C=ncread(filename,'prlr',[1 1 37 3],[144 73 9 1]);
    PRLR_C=reshape(PRLRdata_C,[144 73 9]);
    PRLR_C_1=flipud(rot90(mean(PRLR_C,3)));
    PRLR_05_C(:,:,(s+2)/4)=PRLR_C_1(:,:);
    %读取所有Ensemble数据
    PRLRdata=ncread(filename,'prlr',[1 1 1 3],[144 73 45 1]);
    PRLR=reshape(PRLRdata,[144 73 45]);
    PRLR_1=flipud(rot90(mean(PRLR,3)));
...
for s=4:4:k
    ...
    TimeData=ncread(filename,'leadtime');
    %读取ECWMF数据并平均
    PRLRdata_E=ncread(filename,'prlr',[1 1 1 9],[144 73 9 1]); 
    %取相应模态的值 其中level=850(1), time=June(4)
    PRLR_E=reshape(PRLRdata_E,[144 73 9]);
    PRLR_E_1=flipud(rot90(mean(PRLR_E,3)));%画图格式[73 144 time]
    PRLR_11_E(:,:,(s+0)/4)=PRLR_E_1(:,:);%按照时间填入（应该是+3/4）
    %读取IFM-GEOM数据并平均
    PRLRdata_I=ncread(filename,'prlr',[1 1 10 9],[144 73 9 1]);
    PRLR_I=reshape(PRLRdata_I,[144 73 9]);
    PRLR_I_1=flipud(rot90(mean(PRLR_I,3)));
    PRLR_11_I(:,:,(s+0)/4)=PRLR_I_1(:,:);
    %读取Meteo-France数据并平均
    PRLRdata_M=ncread(filename,'prlr',[1 1  19 9],[144 73 9 1]);
    PRLR_M=reshape(PRLRdata_M,[144 73 9]);
    PRLR_M_1=flipud(rot90(mean(PRLR_M,3)));
    PRLR_11_M(:,:,(s+0)/4)=PRLR_M_1(:,:);
    %读取UK_Met_Office数据并平均
    PRLRdata_U=ncread(filename,'prlr',[1 1 28 9],[144 73 9 1]);
    PRLR_U=reshape(PRLRdata_U,[144 73 9]);
    PRLR_U_1=flipud(rot90(mean(PRLR_U,3)));
    PRLR_11_U(:,:,(s+0)/4)=PRLR_U_1(:,:);
    %读取CMCC-Bologna数据并平均
    PRLRdata_C=ncread(filename,'prlr',[1 1 37 9],[144 73 9 1]);
    PRLR_C=reshape(PRLRdata_C,[144 73 9]);
    PRLR_C_1=flipud(rot90(mean(PRLR_C,3)));
    PRLR_11_C(:,:,(s+0)/4)=PRLR_C_1(:,:);
    %读取所有Ensemble数据
    PRLRdata=ncread(filename,'prlr',[1 1 2 9],[144 73 45 1]);
    PRLR=reshape(PRLRdata,[144 73 45]);
    PRLR_1=flipud(rot90(mean(PRLR,3)));
...
ncid2=netcdf.create('./Ensemble/228EnsembleJuly.nc','clobber');
