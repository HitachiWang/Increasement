...
ncdisp('D:/data/Ensemble/139.ref11.DJF.nc');
ncdisp('D:/data/Ensemble/slp.mon.mean.nc');

G=squeeze(ncread('d:/data/Ensemble/129.ref11.DJF.nc','TS',[1 1 1 1],[144 73  46 5])); 
OBS=squeeze(ncread('d:/data/Ensemble/hgt.mon.mean.nc','hgt'));
...
Ensemble=Ens(:,:,:,1);
for ENS=1:5
    Ensemble=Ensemble+Ens(:,:,:,ENS)-272.15;
end
Ensemble=Ensemble./5;
% Ensemble=Ens(:,:,:,ENS);
AVE=mean(Ensemble(:,:,1:46),3);
OAVE=mean(Obs(:,:,1:46),3);
%观测距
...
sst1=squeeze(mean(mean(Obs(73:110,33:41,:),1),2));
sst2=squeeze(mean(mean(Ensemble(73:110,33:41,:)+272.15,1),2));
plot(1:46,sst1,1:46,sst2);
